package me.bobulo.mine.pdam.feature.packet.metadata.entity;

import me.bobulo.mine.pdam.feature.packet.metadata.entity.acessor.DataAccessor;
import org.jetbrains.annotations.NotNull;

public class BlazeMetadata extends LivingEntityMetadata {

    // Metadata index 16
    public boolean blazeOnFire;

    @Override
    public void populate(DataAccessor accessor) {
        Populator.INSTANCE.populate(this, accessor);
    }

    public static class Populator<T extends BlazeMetadata> extends LivingEntityMetadata.Populator<T> {

        public static final Populator<BlazeMetadata> INSTANCE = new Populator<>();

        protected Populator() {
        }

        @Override
        public void populate(@NotNull T metadata, @NotNull DataAccessor accessor) {
            super.populate(metadata, accessor);

            metadata.blazeOnFire = accessor.getBoolean(16);
        }
    }

}

